...
import logoImg from "../../../assets/rayhan-vai-logo-copyright-2.png";
...
const Header = () => {
  ...
  return (
<header className="fixed top-0 w-full z-50 md:bg-black/30 md:backdrop-blur-sm">
  ...
  {/* Mobile Sidebar */}
  <div
    className={`fixed top-0 right-0 h-screen w-72 bg-[#0f0f0f] border-l border-white/10 z-[60] transition-transform duration-500 ${
  ...
  >
    <div className="flex justify-end p-6">
      <button
        ...
        className="text-[#F6c543] text-3xl hover:rotate-90 transition-transform duration-300"
      >
        <FaTimes />
      </button>
    </div>
      ...
      <div className="flex justify-center gap-6 text-2xl text-gray-400">
        {socialLinks.map((social, index) => (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="hover:text-[#F6c543] transition-colors"
          >
            {social.icon}
          </a>
        ))}
      </div>
      <p className="text-center text-[10px] text-gray-600 mt-6 tracking-widest uppercase font-bold">
        Rayhan Portfolio
      </p>
    </div>
  </div>
