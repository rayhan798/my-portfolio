...
import resumepdf from "../../../assets/RAYHAN_Resume.pdf";

const ResumeCard = ({ data, isLeft, index }) => {
  return (
    <div
      className={`relative flex items-center justify-between w-full mb-12 lg:mb-20 group ${isLeft ? "lg:flex-row" : "lg:flex-row-reverse"}`}
    >
      ...
      >
        ...
        >
          {/* Year Badge */}
          <div
            className={`mb-4 flex ${isLeft ? "lg:justify-end" : "justify-start"}`}
          >
          ...
          {/* Title - Italic Fixed */}
          <h4
            className={`text-xl md:text-2xl font-black text-white mb-2 uppercase italic tracking-tight group-hover/card:text-[#F6c543] transition-colors ${isLeft ? "lg:text-right" : "lg:text-left"}`}
          >
          ...
          <p
            className={`text-[#F6c543]/90 font-bold text-[12px] md:text-[13px] mb-3 tracking-[0.15em] uppercase italic ${isLeft ? "lg:text-right" : "lg:text-left"}`}
          >
          ...
          {/* Description */}
          <p
            className={`text-gray-400 text-xs md:text-sm leading-relaxed font-medium italic opacity-80 group-hover/card:opacity-100 transition-opacity ${isLeft ? "lg:text-right" : "lg:text-left"}`}
          >
            {data.description}
          </p>
        </motion.div>
...
const Resume = () => {
  return (
    <section
      id="resume"
      className="w-full py-20 lg:py-32 bg-black text-white relative overflow-hidden"
    >
      {/* Background Glow */}
